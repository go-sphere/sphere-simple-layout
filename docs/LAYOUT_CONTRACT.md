...
- Wire output and any other generated artifact: generated code.

Proto is the API source of truth. Constructor or provider changes must be
followed by Wire generation. This layout has no persistence layer, so no schema
generation step applies. Make targets are the public development workflow and
must not silently point at missing scripts.
...
```json
{
  "schema_version": 1,
  "name": "simple",
  "repository": "https://github.com/go-sphere/sphere-simple-layout.git",
  "ref": "master",
  "upstream_module": "github.com/go-sphere/sphere-simple-layout",
